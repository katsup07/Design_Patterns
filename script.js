...
const languagePacks = {
  en: {
    htmlLang: 'en',
    tagline: 'TypeScript-focused explanations',
    categoryTitle: (category) => `${category} Patterns`,
    categoryLabel: (category) => `${category} Pattern`,
    overviewTitle: 'Overview',
    whenToUse: 'When to use it',
    structureHeading: 'Structure Overview',
    benefitsTitle: 'Benefits & Trade-offs',
    benefitsHeading: 'Benefits',
    downsidesHeading: 'Downsides',
    examplesTitle: 'TypeScript Examples',
    copyCode: 'Copy code',
    copied: 'Copied!',
  ...
  ja: {
    htmlLang: 'ja',
    tagline: 'TypeScript を中心とした解説',
    categoryTitle: (category) => `${category} パターン`,
    categoryLabel: (category) => `${category} パターン`,
    overviewTitle: '概要',
    whenToUse: '利用するタイミング',
    structureHeading: 'Structure Overview',
    benefitsTitle: '利点とトレードオフ',
    benefitsHeading: '利点',
    downsidesHeading: '欠点',
    examplesTitle: 'TypeScriptの例',
    copyCode: 'コードをコピー',
    copied: 'コピーしました！',
    copyFallback: 'Ctrl+C を押してください',
  },
};

let currentLanguage = 'en';
let activePatternId = null;
const navGroupTitles = new Map();

function getStrings() {
  return languagePacks[currentLanguage];
}
...
function applyLanguageToStatic() {
  ...
  navGroupTitles.forEach((title, category) => {
    title.textContent = strings.categoryTitle(category);
  });
}
...
function setLanguage(language) {
  if (!languagePacks[language] || language === currentLanguage) {
    return;
  }
  currentLanguage = language;
  applyLanguageToStatic();
  setActivePattern(activePatternId ?? patterns[0].id);
...
categories.forEach((items, category) => {
  ...
  title.textContent = getStrings().categoryTitle(category);
  group.appendChild(title);
  navGroupTitles.set(category, title);
...
function setActivePattern(id) {
  ...
  categoryEl.textContent = strings.categoryLabel(pattern.category);

  summaryEl.innerHTML = `
    <h3>${strings.overviewTitle}</h3>
    <p>${pattern.description}</p>
    <div class="pattern-overview">
      <div class="pattern-meta">
        <div class="pattern-meta__section">
          <h4>${strings.whenToUse}</h4>
      ...
      <div class="pattern-diagram">
        <h4 class="structure-heading">${strings.structureHeading}</h4>
  ...
  benefitsEl.innerHTML = `
    <h3>${strings.benefitsTitle}</h3>
    <div class="benefits-grid">
      <div>
        <h4>${strings.benefitsHeading}</h4>
        <ul>
          ${pattern.benefits.map((item) => `<li>${item}</li>`).join('')}
        </ul>
      </div>
      <div>
        <h4>${strings.downsidesHeading}</h4>
        <ul>
...
function renderExamples(pattern) {
  ...
  header.textContent = strings.examplesTitle;
  ...
  examples.forEach((example, index) => {
    ...
    copy.addEventListener('click', async () => {
      try {
        ...
        copy.textContent = strings.copyFallback;
        setTimeout(() => (copy.textContent = strings.copyCode), 1500);
      }
    });
